...
export function buildCountryFaqGroups({ country, bestPlan, networks, coverage, plans = [] }) {
  ...
  return [
    { id: "plans", label: "Plans and pricing", summary: "Cost, data and validity", questions: [
      { number: "01", question: `Which verified eSIM offer is shown for ${country}?`, answer: `${bestPlan.brand} ${bestPlan.product} is ranked first: ${bestPlan.dataLabel}, ${bestPlan.days} days and a published price of $${bestPlan.price.toFixed(2)}. The comparison currently contains ${displayedPlans.length} source-checked package${displayedPlans.length === 1 ? "" : "s"}.` },
